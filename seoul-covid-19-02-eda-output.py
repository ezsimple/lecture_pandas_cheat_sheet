...
# %%
value_counts = df["확진일자"].value_counts()
# 확진일자로 선그래프를 그립니다.
value_counts.sort_index().plot(figsize=(15, 3))
# 평균값 구하기
mean = value_counts.describe().mean()
# 평균값을 라인으로 표시하기
...
# 평균값을 라인으로 표시하기
plt.axhline(mean, color="red", linestyle=":")
# 루프를 통해 일자별 확진자수를 선그래프로 그립니다.
for i in range(len(day_count)):
    case_count = day_count.iloc[i]
    if case_count > 20: # 너무 많이 출력되는 걸 막기 위해 20보다 큰 경우만 그립니다.
...
# 중위값 구하기
median = day_count.median()

# 중위값을 라인으로 표시하기
g.axhline(median, linestyle=":", color="red")
for i in range(50):
    case_count = day_count[-50:].iloc[i]
    if case_count > median:
...
# %%
# 월-주 함께 그리기
# groupby 를 통해 "월", "주" 로 그룹화 하여 빈도수를 계산합니다. (중요)
month_weekly_case = df.groupby(["월", "주"])["연번"].count()
month_weekly_case

# size().unstack() => dataframe을 만들어줍니다.
df.groupby(["월", "주"]).size().unstack().plot.bar(figsize=(15, 4), rot=0)
...
# %%
# 확진자가 없는 날은 데이터프레임에 어떻게 만들까?
# 모든 날짜를 행에 만들어 주기
# 첫 확진일 부터 마지막 확진일까지 가져옵니다.
# 데이터프레임의 첫번째 날짜는 first_day 에 마지막 날짜는 last_day 에 담습니다.
first_day = df.iloc[-1, 7] # iloc는 콤마로 위치를 지정합니다.

# %%
# %timeit last_day = df.iloc[0]["확진일자"]으로, 처리시간을 측정할 수 있습니다.
# df.iloc[0, 7] .vs. df.iloc[0]["확진일자"]를 비교하면, 처리시간이 많이 차이납니다ㅣ;
...
# days.to_frame()으로도 변경할 수 있습니다.
df_days = pd.DataFrame({"확진일자": days}) # 컬럼명을 확진일자로 지정합니다.
df_days.head()
...
df_daily_case = daily_case.to_frame() # to_frame()을 통해 데이터프레임으로 변환합니다.
df_daily_case.head()
...
# SQL의 join과 유사한 기능입니다. (중요)
# left join을 통해서 확진자가 없는 날을 전체 일자에 표현이 되도록 합니다.
# df_days 를 left, df_daily_case 를 right로 merge 합니다.
# jupyter notebook에서는 shift+tab으로 도움말을 볼 수 있습니다.
...
all_day["일자"] = all_day["확진일자"].astype(str).map(lambda x : x[-5:]) # 스트링의 슬라이싱 기능을 사용
all_day.head()

# %%
# "확진수", "누적확진" 컬럼을 갖는 데이터프레임을 만듭니다.
cum_day = all_day[["일자", "확진수", "누적확진"]]
cum_day = cum_day.set_index("일자") # 시각화 하기 위해서, 인덱스를 일자로 지정합니다.
cum_day.head()
...
# %%
# 시리즈로 2개의 그래프 그리기 (중요)
# 시리즈로 그래프를 그리는 것은, 여러 그래프를 하나의 그래프에 그릴 수 있는 기능입니다.
cum_day["확진수"].plot()
...
all_day_week = all_day_week.astype(int) # 소숫점 대신 정수형으로 보고 싶습니다.
# type(all_day_week) => unstack() 처리후 DataFrame으로 반환됩니다.
all_day_week

# %%
# 숫자로 표현된 요일을 문자로 바꿔주기 위해 split 을 통해 문자를 리스트로 변경합니다.
dayofweek = "월 화 수 목 금 토 일" # 자주 사용될 방법 (중요)
dayofweek = dayofweek.split() # 기본이 공백을 split합니다.
dayofweek
...
# %%
# pandas의 style 기능을 이용해서 시각화를 합니다.
# style.background_gradient 로 색상을 표현합니다.
# 높은 숫자는 진하게 표시, Blues_r : 작은 수가 진하게 표시
all_day_week.style.background_gradient(cmap="Blues")

# %%
# 카테고리형 데이터에 대한 분석
# 거주지(구별) 확진자의 빈도수를 구하고 시각화 합니다.
# 시리즈 형태의 데이터 요약은 value_counts() 사용
gu_count = df["거주지"].value_counts()
gu_count.sort_values()

# %%
# 구별 확진자의 수를 시각화 합니다.
# sort_values()는 낮은 숫자부터 출력합니다.(마지막은 가장 높은 수)
# barh()는 마지막부터 그립니다.(높은 수부터 그립니다.)
...
# pd.DataFrame(gu_count), gu_count.to_frame(), gu_count.reset_index() 차이점을 알아봅시다.
...
# .loc[행, 열]
# .loc[조건, 열]
# 원본을 회손하지 않고 "지역" 컬럼 추가 하기
df.loc[df["거주지"].isin(gu), "지역"] = df["거주지"]
# df.loc[~df["거주지"].isin(gu)] # 해당되지 않는 지역(타지역)
# df.loc[df["지역"].isnull(), "지역"] = "타지역"
df["지역"] = df["지역"].fillna("타지역") # 결측치만 타지역으로 채워줌 (중요)
df["지역"].unique()
...
contact_count_top = contact_count.sort_values().tail(30) # 상위 30개만 가져옵니다.
...
# groupby, pivot_table, cross_table 등을 이용해서 구할 수 있습니다.
# unstack을 이용하면, 마지막 값을 index로 하여 DataFrame을 만들게 됩니다.
...
# %%
unknown_weekly_case.plot.bar(figsize=(15, 4), rot=0)

# %%
# 전체 확진수를 value_counts 로 구하고 데이터프레임 형태로 만듭니다.
# type(df["주"].value_counts()) # Series
...
# 로그스케일 (중요)
# 차이가 너무 커서 그래프가 자세히 보이지 않을때, 로그스케일로 표현하면 차이가 큰 값의 스케일을 조정해주게 됩니다.
np.log(unknown_case["전체확진수"]).plot.line(figsize=(15, 4), rot=0)
np.log(unknown_case["불명확진수"]).plot.line()
...
# %%
# 어떤 값이 있는지 확인하기
df["접촉력"].map(lambda x: x)

# %%
# 함수를 map을 통해 접촉번호를 구합니다.
df["접촉번호"] = df["접촉력"].map(get_number)
contact = df["접촉번호"].value_counts().reset_index() # .reset_index()를 통해 DataFrame으로 변환
contact.head()
...
df_contact = contact.drop(0, axis=0) # 접촉번호가 없는 0번 행을 drop 으로 삭제합니다.(기본은 행을 기준 (axis=0))
...
top_contact_no.sort_values(ascending=False)
...
# normalize 는 빈도수를 모든 값의 합으로 나눠서 계산합니다.(비율)
...
# .sort_values().plot.barh() 빈도수가 많은 것이 위로 오게 됩니다.
...
# %%
hospital_gu_count = df.groupby(["지역", "병원"])["연번"].count()
hospital_gu_count
...
# %%
# iloc로 일부 데이터만 보기
# 전체 데이터를 보기에 너무 많을 수 있기 때문에
# iloc 를 사용해서 일부 데이터만 봅니다. (중요)
hospital_gu.fillna(0).iloc[:8, :8]

# %%
# loc로 일부 데이터만 보기 (행 인덱스로 가져오기)
...
hospital_gu_heatmap[top_hospital_index].style.background_gradient(cmap="Reds")

# %%
# 전체 병원이 너무 많기 때문에 환자가 많은 병원만 따로 봅니다.
hospital_gu_heatmap[top_hospital_index].T.style.background_gradient(cmap="Reds")

# %%
# 여행력
# "해외" 라는 컬럼을 만들어 데이터를 전처리 합니다.
# "여행력"에 "-" 문자가 들어가 있으면 결측치로 처리합니다.
# str.replace() 는 일부가 일치할 경우 치환이 됩니다.
df["해외"] = df["여행력"]
df["해외"] = df["해외"].str.strip()
df["해외"] = df["해외"].replace("-", np.nan) # .notnull()을 사용하기 위해, NaN으로 바꿔줍니다. (중요)
df["해외"].unique()
...
# (중요)
# 원본(df)를 보호하기 위해 .copy() 를 사용합니다.
# .copy()를 사용하여 복사하여 새로운 데이터프레임을 만듭니다.
# .copy()를 사용하지 않을 경우, 복사하려는 데이터프레임이 참조되어 데이터를 변경하면 원본 데이터프레임이 변경됩니다.
...
# %%
southeast_asia = "필리핀, 방글라데시, 인도네시아, 태국, 말레이시아, 마닐라"
southeast_asia = southeast_asia.replace(", ", "|")
southeast_asia

# %%
eurasia = "러시아, 벨라루스, 카자흐스탄, 키르기스스탄, 타지키스탄, 아르메니아"
eurasia = eurasia.replace(", ", "|")
eurasia
# %%
df_oversea["해외"].unique()
...
df_oversea.loc[df_oversea["해외"].str.contains(eurasia), "해외"] = "유라시아"
df_oversea.loc[df_oversea["해외"].str.contains(south_america), "해외"] = "남미"
df_oversea.loc[df_oversea["해외"].str.contains(southeast_asia), "해외"] = "동남아"
...
df_day_oversea = df_day_oversea.rename(columns={"연번":"누적확진수"}) # 연번을 누적확진자수로 변경
df_day_oversea.head()
...
# 이미 확진일자가 index 일 경우, Excepiton 을 던집니다.
# df_day_oversea = df_day_oversea.set_index("확진일자")

df_day_oversea

# pivot에서 index로 사용할 컬럼을 지정합니다.
# pivot_table은 연산이 필요할 경우 사용합니다.
df_day_oversea.pivot(columns="해외").plot(figsize=(24, 8), legend=True)
...
# %%
# 해외유입확진수 구하기 (merge를 위해 DataFrame을 생성)
...
# left_index, right_index 옵션을 사용하여 기본적으로 index를 기준으로 merge 합니다.
df_all_oversea_case = df_all_gu.merge(df_oversea_gu,
            left_index=True, right_index=True,
            how="left")
...
df_all_oversea_case.sort_values(by=["해외유입비율", "해외유입확진수"], ascending=False).head(10)

# %%
# 시각화 하기
# by는 정렬 기준
# 라인그래프의 경우 모든 데이터 출력을 하지 못하고 있습니다.
df_all_oversea_case.sort_values(by="해외유입확진수").plot.line(figsize=(15, 8))

df_all_oversea_case.sort_values(by=["해외유입확진수", "해외유입확진수"]).plot.barh(figsize=(15, 8))


# %%
# 13.6 해외유입 확진자중 퇴원 여부
...
oversea_finish_count.plot.bar(figsize=(15, 4), rot=30, color=["#ff0000", "#0000ff"])

# %%
# 월별 해외 확진자 수
oversea_monthly_case = df_oversea["월"].value_counts()
type(oversea_monthly_case) # Series

# %%
oversea_monthly_case.sort_index().plot() # 라인그래프로 그리인
oversea_monthly_case.sort_index().plot.bar(rot=0)

# %%
### 구와 월별 해외 확진자 수
# groupby vs crosstab vs pivot vs pivot_table 의 차이는?
# groupby, crosstab, pivot_table 로 같은 결과의 테이블이 나오도록 구합니다.
#### groupby 로 빈도수 구하기
# unstack() 를 통해 두번째 컬럼을 피벗으로 사용하여 구합니다.
# groupby(["월", "지역"])["연번"].count()를 unstack()하면,
# 행(월), 열(지역), 값(연번.count())으로 변환됩니다.
month_gu = df_oversea.groupby(["월", "지역"])["연번"].count().unstack()
month_gu = month_gu.fillna(0).astype(int)
month_gu.style.background_gradient(cmap="Reds") # cmap : color map
...
# %%
# pivot 은 모양만 바꿉니다.
# pivot_table 은 모양과 값을 바꿉니다. (aggregate 함수를 사용하여 값을 구합니다.)
...
type(top_oversea_gu) # Series

# %%
# 시각화 하기
g = month_gu[top_oversea_gu].plot(figsize=(15, 4))
g.legend(loc=0) # loc : legend location, 0:best, 1:right
...
group_oversea_gu.T.fillna(0).astype(int).style.background_gradient()
...
df_group_oversea_gu = group_oversea_gu[top_oversea_gu].dropna(how="all").fillna(0).astype(int)
df_group_oversea_gu.style.background_gradient(cmap="Reds") # cmap : color map
